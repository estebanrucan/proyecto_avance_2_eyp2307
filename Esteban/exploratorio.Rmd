...
a <- coefficients(backward) %>% round(., 4) %>% as.data.frame()
names(a) <- c("coef")
a <- a %>% arrange(coef)

b <- coef(ridge_best) %>% as.matrix() %>% as.data.frame() %>% round(., 6) %>% arrange(s0)

c <- coef(lasso_best) %>% as.matrix() %>% as.data.frame() %>% round(., 6) %>% arrange(s0)
