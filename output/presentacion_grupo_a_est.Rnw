...
<<setup, include=FALSE>>=
options(width=60)
opts_chunk$set(fig.width = 7, fig.height = 4, fig.align='center', concordance=TRUE, fig.show='hold', size='footnotesize', prompt=FALSE, comment=NA, tidy=TRUE, results="markup", tidy.opts=list(width.cutoff=50), comment="", highlight=TRUE, warning = FALSE, message = FALSE)
...
sumario = df %>% summarise(Minimo = min(critical_temp),
                           ...
                           Dev_Estandar = sd(critical_temp)) %>% t() %>%
    xtable()
names(sumario) = c("critical_temp")
rownames(sumario) = c("Mínimo","Primer Cuartil","Mediana","Tercer Cuartil","Máximo", "Promedio", "Desv. Estándar")
