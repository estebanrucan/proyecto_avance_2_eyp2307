...
<<setup, include=FALSE>>=
options(width=60)
opts_chunk$set(fig.width = 7, fig.height = 4, fig.align='center', concordance=TRUE, fig.show='hold', size='footnotesize', prompt=FALSE, comment=NA, tidy=TRUE, results="markup", tidy.opts=list(width.cutoff=50), comment="", highlight=TRUE, warning = FALSE, message = FALSE, echo = FALSE)
opts_knit$set(concordance=TRUE,self.contained=FALSE)
require(tidyverse)
@

<<>>=
load("D:\\EYP2307\\proyecto_avance_2_eyp2307\\output\\ws\\info.RData")
@
...
ggplot() +
    ...
    geom_density(aes(x = fitted_values_bw, color = "darkorange"), size = 1.3,alpha = 0.3) +
    ylab("Densidad") +
    xlab("Valores") +
    xlim(0, 150) +
    ylim(0,0.05) +
    scale_color_identity(name = "Variable:", breaks = c("dodgerblue", "darkorange"), labels = c("crit_temp","Av. 1"), guide = "legend") +
    theme_bw()
@
